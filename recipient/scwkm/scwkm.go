...
func (r *Recipient) WrapKey(ctx context.Context, dek []byte) (*recipient.WrappedKey, error) {
	_ = ctx
	_ = dek
	return nil, enigma.WrapError("scwkm.WrapKey", enigma.ErrNotImplemented, fmt.Errorf("cloud integration is deferred in v1"))
}

func (r *Recipient) UnwrapKey(ctx context.Context, wk *recipient.WrappedKey) ([]byte, error) {
	_ = ctx
	_ = wk
	return nil, enigma.WrapError("scwkm.UnwrapKey", enigma.ErrNotImplemented, fmt.Errorf("cloud integration is deferred in v1"))
}
